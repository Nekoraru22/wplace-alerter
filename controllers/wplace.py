...
class WPlaceArtInterface(BaseModel):
    ...
    last_checked: str = ""
    griefed: bool = False
    api_image: str = Field(..., pattern=r'^https://backend\.wplace\.live/files/s0/tiles/\d+/\d+\.png$') # ^https://backend\.wplace\.live/tile/\d+/\d+\.png$
...
class WPlace:
    ...
    def get_tiles_from_api_url(self, api_image: str) -> Tuple[int, int]:
        ...
        """
        match = re.search(r'https://backend\.wplace\.live/files/s0/tiles/(\d+)/(\d+)\.png', api_image) # https://backend\.wplace\.live/tile/(\d+)/(\d+)\.png
        if match:
